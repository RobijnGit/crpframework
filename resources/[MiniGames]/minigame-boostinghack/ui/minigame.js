...
window.onload = ('message', (e) => {
    window.addEventListener('message', (event) => {
        if (event.data.action === 'start') {
            start(event.data.allowed, event.data.timer);
        }
    });
})
...
function check() {
    ...
    if (game_started && current_attempt === correct_pos) {
        // success
        $.post(`https://minigame-boostinghack/NUI:close`, JSON.stringify({
            'success': true
        }));
        setTimeout(() => {
...
function start(allowed, timer) {
    document.getElementById('html').style.display = "";
    codes_pos = 0;
    current_pos = 43;

    let randomChoice = allowed || ['numeric', 'alphabet', 'alphanumeric', 'greek', 'braille', 'runes', 'symbols']
