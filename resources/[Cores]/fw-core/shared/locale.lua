...
function Locale:t(key, subs)
    ...
    -- plural stuff by r3ps4j
    if type(subs.count) == "number" then
        local suffix = "_other"
        if subs.count == 0 then
            suffix = "_zero"
        elseif subs.count == 1 then
            suffix = "_one"
        end
        local newKey = key..suffix

        if type(self.phrases[newKey]) == "string" then
            return translateKey(self.phrases[newKey], subs)
        end
    end
