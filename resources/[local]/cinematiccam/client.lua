...
local nearDof = 0.7
local itemNearDof
local nearDofCounter = 0
local nearDof = 1.0
local nearDofCurr
local nearDofs = {}
for i = 0.0, 50.0, 0.1 do
    table.insert(nearDofs, tostring(i))
    nearDofCounter = nearDofCounter + 1
    if (tostring(i) == "0.7") then
        nearDofCurr = nearDofCounter
    end
end
...
local farDof = 0.7
local itemFarDof
local farDofCounter = 0
local farDof = 1.0
local farDofCurr
local farDofs = {}
for i = 0.0, 50.0, 0.1 do
    table.insert(farDofs, tostring(i))
    farDofCounter = farDofCounter + 1
    if (tostring(i) == "1.3") then
        farDofCurr = farDofCounter
    end
end
...
function GenerateCamMenu()
    ...
    local itemToggleDof = NativeUI.CreateCheckboxItem("Enable DoF", dofEnabled, "Enable Depth of Field")
    camMenu:AddItem(itemToggleDof)
    itemToggleDof.CheckboxEvent = function(menu, item, checked)
        ToggleDof(checked)
    end

    itemNearDof = NativeUI.CreateListItem("Near Dof", nearDofs, nearDofCurr, "This sets at what distance your camera should start to focus (Example: 0.7 meters)")
    camMenu:AddItem(itemNearDof)

    itemNearDof.OnListChanged = function(menu, item, newindex)
        ChangeDof(newindex)
    end

    itemFarDof = NativeUI.CreateListItem("Far Dof", farDofs, farDofCurr, "This sets at what distance your camera should stop to focusing (Example: 1.3 meters)")
    camMenu:AddItem(itemFarDof)

    itemFarDof.OnListChanged = function(menu, item, newindex)
        ChangeFarDof(newindex)
    end
...
function ToggleDof(checked)
    dofEnabled = checked
    -- SetCamUseShallowDofMode(cam, checked)
...
function StartFreeCam(fov)
    ...
    SetCamNearDof(cam, 10.0) -- Sets the camera's near dof to the value set in the config file
    SetCamFarDof(cam, 25.0) -- Sets the camera's far dof to the value set in the config file
    SetCamDofStrength(cam, 0.3) -- Sets the camera's dof strength to the value set in the config file
...
function ChangeDof(newindex)
    nearDof   = itemNearDof.Items[newindex]
    nearDofCurr  = newindex
    SetCamNearDof(cam, nearDof) print('nearDof', nearDof)
    ToggleDof(true)
end

function ChangeFarDof(newindex)
    farDof   = itemFarDof.Items[newindex]
    farDofCurr  = newindex
    SetCamFarDof(cam, farDof) print('farDof', farDof)
    ToggleDof(true)
end
