    ...
    for k, v in pairs(Items) do
        ...
        elseif v.item_name == 'hunting-carcass-four' then
            ...
            if (Time.Hour >= 20 and Time.Hour <= 23) or (Time.Hour >= 0 and Time.Hour <= 6) then
                ...
                Player.Functions.Notify('Will you come back tonight? I\'m not going to accept these kinds of things during the day...', 'error')
            end
        end
    end

    if TotalReceive > 0 then
        Player.Functions.Notify('Added to your bank balance.')
        exports['fw-financials']:AddMoneyToAccount('1001', '1', Player.PlayerData.charinfo.account, TotalReceive, 'SALES', 'Job: Hunting Sales')
    end
end)
