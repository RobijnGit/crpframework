...
--                 --     Citizen.CreateThread(function()
--                     --         PlayAmbientSpeech1(PlayerPedId(), "GENERIC_CURSE_MED", "SPEECH_PARAMS_FORCE")
--                     --         TriggerEvent("fw-police:Client:InCamera", true)
--             --     end)
--             -- end
...
--             SetNightvision(false)
--             SetSeethrough(false)
--             TriggerEvent("fw-police:Client:InCamera", false)
--         end
--     end
-- end)
...
-- Activate binoculars
RegisterNetEvent('fw-items:binoculars:toggle')
AddEventHandler('fw-items:binoculars:toggle', function()
...
RegisterNetEvent('fw-items:camera:toggle')
AddEventHandler('fw-items:camera:toggle', function()
    camera = not camera

    TriggerEvent("fw-police:Client:InCamera", true)

    if not camera then
        StopAnimTask(PlayerPedId(), "amb@world_human_paparazzi@male@base", "base", 1.0)
        exports['fw-assets']:RemoveProp("PaparazziCamera")
        return
    end

    exports['fw-assets']:RequestAnimationDict("amb@world_human_paparazzi@male@base")
    TaskPlayAnim(PlayerPedId(), "amb@world_human_paparazzi@male@base", "base", 1.0, 1.0, -1, 1, 1.0, false, false, false)

    exports['fw-assets']:AddProp("PaparazziCamera")
    ...
    local LastScaleformUpdate = GetGameTimer() - 5000

    Citizen.CreateThread(function()
        while camera do

            if GetGameTimer() - LastScaleformUpdate > 5000 then
                LastScaleformUpdate = GetGameTimer()
            ...
            if IsControlJustReleased(0, storeBinoclarKey) then -- Toggle camera
                TriggerEvent("fw-items:camera:toggle")
                break
            end

            if not IsEntityPlayingAnim(PlayerPedId(), "amb@world_human_paparazzi@male@base", "base", 1) then
                TaskPlayAnim(PlayerPedId(), "amb@world_human_paparazzi@male@base", "base", 1.0, 1.0, -1, 1, 1.0, false, false, false)
            end
