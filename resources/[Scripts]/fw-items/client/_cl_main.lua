    ...
    if not exports['fw-progressbar']:GetTaskBarStatus() then
        if not DoingSomething then
            ...
            Citizen.SetTimeout(1000, function()
                ...
                FW.Functions.Progressbar("mint", "Taking a mint..", 6000, false, true, {
                    disableMovement = false,
                    disableCarMovement = false,
                    disableMouse = false,
                    disableCombat = true,
...
AddEventHandler("fw-items:client:use:lighter", function()
    if not LigherIsHot then
        ...
        FW.Functions.Notify("Wait a little..", "error")
    end
end)
...
AddEventHandler('fw-items:client:use:tirekit', function()
    ...
    if not exports['fw-progressbar']:GetTaskBarStatus() then
        ...
        if Vehicle ~= -1 then
            exports["fw-inventory"]:SetBusyState(true)
            FW.Functions.Progressbar("repair_vehicle", "Changing tyres..", math.random(10000, 15000), false, true, {
...
AddEventHandler('fw-items:Client:PickupWheelchair', function(Data, Entity)
    if IsVehicleSeatFree(Entity, -1) then
        ...
        FW.Functions.Notify("How dare you take a wheelchair away from a cripled person...", "error")
    end
end)
...
AddEventHandler('fw-items:Client:Used:Scootmobile', function()
    if not IsPedInAnyVehicle(PlayerPedId()) then
        FW.Functions.Progressbar("remove-armor", "Placing Scootmobile..", 2000, false, false, {
            disableMovement = true,
            disableCarMovement = false,
            disableMouse = false,
            disableCombat = true,
...
AddEventHandler('fw-items:Client:PickupScootmobile', function(Data, Entity)
    if IsVehicleSeatFree(Entity, -1) then
        ...
        FW.Functions.Notify("Someones is still riding the scootmobile...", "error")
    end
end)
...
AddEventHandler('fw-items:Client:SetMoneyCase', function(Slot, IsAdv)
    ...
    local Result = exports['fw-ui']:CreateInput({
        { Label = 'Balance', Icon = 'fas fa-dollar-sign', Name = 'Worth', Type = 'number' },
        { Label = 'Code (optioneel)', Icon = 'fas fa-ellipsis-h', Name = 'Code', Type = 'password' },
    })

    if Result then
        if tonumber(Result.Worth) <= 0 then FW.Functions.Notify("You need to at least put $1 in.", "error") return end
        if tonumber(Result.Worth) > MaxWorth then FW.Functions.Notify("You can only fit $" .. (IsAdv and "100.000" or "50.000") .. " in this case..", "error") return end
