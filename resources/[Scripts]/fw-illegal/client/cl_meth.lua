...
AddEventHandler("fw-illegal:Client:Meth:PlaceTable", function(Item)
    ...
    if not exports['fw-inventory']:HasEnoughOfItem('methtable', 1) then
        return FW.Functions.Notify("Where's your table?", "error")
    end
    ...
    if exports['fw-interiors']:IsInsideInterior() then
        IsPlacing = false
        return FW.Functions.Notify("You can't stand the chemical smell in here..", "error")
    end
...
AddEventHandler("fw-illegal:Client:PickupMethTable", function(Data, Entity)
    ...
    local Finished = FW.Functions.CompactProgressbar(30000, "Picking up...", false, true, {disableMovement = false, disableCarMovement = false, disableMouse = false, disableCombat = true}, { animDict = "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", anim = "machinic_loop_mechandplayer", flags = 1 }, {}, {}, false)
	StopAnimTask(PlayerPedId(), "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", "machinic_loop_mechandplayer", 1.0)
...
AddEventHandler("fw-illegal:Client:StartCookingMeth", function(Data, Entity)
    if IsCooking then
        return FW.Functions.Notify("You're already cooking..")
    end
    ...
    if TableData.Owner ~= FW.Functions.GetPlayerData().citizenid then
        return FW.Functions.Notify("This ain't yours?")
    end

    if TableData.LastCook + ((60 * 60) * 1.5) > GetCloudTimeAsInt() then -- 1 hour and 30 minutes
        return FW.Functions.Notify("This table is still cooling back down..", "error")
    end
...
AddEventHandler("fw-illegal:Client:DoMethCook", function(Data, Entity)
    ...
    if Data.Stage == 1 then
        Time, Text, Dict, Anim = 30000, "Setting up the goods", "anim@heists@prison_heiststation@", "pickup_bus_schedule"
    elseif Data.Stage == 2 then
        Time, Text, Dict, Anim = 45000, "Combining the goods", "anim@heists@prison_heiststation@cop_reactions", "cop_b_idle"
        if ExplodeThisCook and math.random(1, 100) > 80 then DoExplode, ExplodeThisCook = true, false end
    elseif Data.Stage == 3 then
        Time, Text, Dict, Anim = 25000, "Mixing water", "weapon@w_sp_jerrycan", "fire"
        if ExplodeThisCook and math.random(1, 100) > 70 then DoExplode, ExplodeThisCook = true, false end
    elseif Data.Stage == 4 then
        Time, Text, Dict, Anim = 60000, "Adding solvent", "weapon@w_sp_jerrycan", "fire"
        if ExplodeThisCook and math.random(1, 100) > 50 then DoExplode, ExplodeThisCook = true, false end
    elseif Data.Stage == 5 then
        Time, Text, Dict, Anim = 120000, "Crystallizing the product", "random@train_tracks", "idle_e"
        if ExplodeThisCook and math.random(1, 100) > 30 then DoExplode, ExplodeThisCook = true, false end
    elseif Data.Stage == 6 then
        Time, Text, Dict, Anim = 60000, "Packing the product", "anim@heists@prison_heiststation@", "pickup_bus_schedule"
    end
...
AddEventHandler("fw-illegal:Client:CutMeth", function(Item)
    ...
    local Finished = FW.Functions.CompactProgressbar(1000, "Cutting and packing...", false, true, {disableMovement = false, disableCarMovement = false, disableMouse = false, disableCombat = true}, {}, {}, {}, false)
    if not Finished then return end
...
AddEventHandler("fw-inventory:Client:OnItemInsert", function(FromItem, ToItem)
    ...
    if ToItem.Info.Uses + Quantity > 4 then
        return FW.Functions.Notify("Looks like the pipe is full..", "error")
    end

    exports['fw-inventory']:SetBusyState(true)

    local Finished = FW.Functions.CompactProgressbar(10000, "Packaging into pipe...", false, true, {disableMovement = false, disableCarMovement = false, disableMouse = false, disableCombat = true}, {}, {}, {}, false)
...
AddEventHandler("fw-illegal:Client:UseMethpipe", function(Item)
    ...
    if Uses <= 0 then
        return FW.Functions.Notify("Looks like you're out of meth..", "error")
    end
