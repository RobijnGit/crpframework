...
$(function () {
    window.addEventListener('message', function (event) {
        ...
        if (event.data.type == "refreshcarmenu") {
            ...
            if (settings['engine'] === true) $('.engine').parent().addClass('active');
        }
    });
    ...
    $('.door').on('click', function() {
        ...
        $.post('https://fw-carmenu/openDoor', JSON.stringify({
                doorIndex: doorIndex
            })
        );
    });
    ...
    $('.seat').on('click', function() {
        ...
        $.post('https://fw-carmenu/switchSeat', JSON.stringify({
                seatIndex: seatIndex
            })
        );
    });
    ...
    $('.window').on('click', function() {
        ...
        $.post('https://fw-carmenu/togglewindow', JSON.stringify({
    ...
    $('.engine').on('click', function() {
        if ($(this).parent().hasClass('disabled')) return;
        $.post('https://fw-carmenu/toggleengine', JSON.stringify({}));
    });

    document.onkeyup = function (data) {
        if (!open) return;

        if (data.which == 90 || data.which == 27) {
            $.post('https://fw-carmenu/escape', JSON.stringify({}));
        }
    };
});
